"""Вкладка Streamlit-приложения: сборка видео по образцу монтажа (существующий видео-модуль)."""
...
def render_video_tab() -> None:
    """Отрисовывает вкладку сборки видео по образцу монтажа."""
    ...
    reference_file = st.file_uploader(
        "Видео-референс (образец монтажа)", type=["mp4", "mov", "m4v"], key="video_reference_uploader"
    )
    clip_files = st.file_uploader(
        "Ваши сырые клипы (можно выбрать несколько)",
        type=["mp4", "mov", "m4v"],
        accept_multiple_files=True,
        key="video_clips_uploader",
    )

    start_button = st.button(
        "Собрать видео", type="primary", disabled=not (reference_file and clip_files), key="video_assemble_button"
    )
